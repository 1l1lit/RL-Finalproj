...
def warmup(env, buffers):
    ...
    for i in range(env.n_agents):
        buffers[i].share_obs[0] = obs[i].copy()
...
def use_vmas_env(
    ...
    **kwargs,
    ...
    agents = [Agent(n_agents, obs_dim=16, share_obs_dim=16, action_dim=2, device=device) for _ in range(n_agents)]
    buffers = [ReplayBuffer(n_steps, num_envs, n_agents, act_dim=2, obs_dim=16, share_obs_dim=16) for _ in range(n_agents)]
    ...
    for epoch in range(epochs):
        ...
        for step in range(n_steps):
            ...
            for i in range(n_agents):
                buffers[i].insert(share_obs[i], obs[i], actions[i].detach().numpy(), rews[i].reshape(1,-1), masks[i], value_preds[i].detach().numpy(), action_probs[i].detach().numpy())
        ...
        if epoch % 200 == 0 and epoch != 0 or epoch == epochs - 1:
            torch.save(reward_list, f"result-ippo/reward_list_{epoch}_{version}.pt")
            for i in range(n_agents):
                torch.save(agents[i].actor.state_dict(), f"result-ippo/model/actor_{i}_{epoch}_{version}.pt")
                torch.save(agents[i].critic.state_dict(), f"result-ippo/model/critic_{i}_{epoch}_{version}.pt")
                torch.save(agents[i].actor_optimizer.state_dict(), f"result-ippo/model/actor_optimizer_{i}_{epoch}_{version}.pt")
                torch.save(agents[i].critic_optimizer.state_dict(), f"result-ippo/model/critic_optimizer_{i}_{epoch}_{version}.pt")
